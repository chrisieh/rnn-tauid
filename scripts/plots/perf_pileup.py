...
def load_bdt(fsig, fbkg, branchname):
    sig_bdt = root2array(fsig, treename="CollectionTree",
                         branches=[branchname, "TauJets.pt", "TauJets.mu",
                                   "TauJets.nVtxPU", "weight"])
    ...
    nvtx = np.concatenate([sig_bdt["TauJets.nVtxPU"], bkg_bdt["TauJets.nVtxPU"]])

    return y, y_true, w, pt, mu, nvtx
...
def load_rnn(fsig, fbkg, sig_deco, bkg_deco):
    ...
    with h5py.File(fsig, "r", driver="family", memb_size=10*1024**3) as f:
        ...
        nvtx_sig = f["TauJets/nVtxPU"][sig_idx:]
    ...
    with h5py.File(fbkg, "r", driver="family", memb_size=10*1024**3) as f:
        ...
        nvtx_bkg = f["TauJets/nVtxPU"][bkg_idx:]
    ...
    nvtx = np.concatenate([nvtx_sig, nvtx_bkg])

    return y, y_true, w, pt, mu, nvtx


# BDT
y_bdt_1p, y_true_bdt_1p, w_bdt_1p, pt_bdt_1p, mu_bdt_1p, nvtx_bdt_1p = load_bdt(
    fsig_bdt_1p, fbkg_bdt_1p, branchname_1p)
y_bdt_3p, y_true_bdt_3p, w_bdt_3p, pt_bdt_3p, mu_bdt_3p, nvtx_bdt_3p = load_bdt(
    fsig_bdt_3p, fbkg_bdt_3p, branchname_3p)

# Trk+Cls+MLP
y_trk_cls_mlp_1p, y_true_1p, w_1p, pt_1p, mu_1p, nvtx_1p = load_rnn(
    fsig_1p, fbkg_1p, sig_deco_trk_cls_mlp_1p, bkg_deco_trk_cls_mlp_1p)
y_trk_cls_mlp_3p, y_true_3p, w_3p, pt_3p, mu_3p, nvtx_3p = load_rnn(
    fsig_3p, fbkg_3p, sig_deco_trk_cls_mlp_3p, bkg_deco_trk_cls_mlp_3p)

##### TIGHT #####
# Rejection vs pt
#bins = np.array([0, 8, 10, 12, 14, 16, 18, 20, 22, 24, 28, 40], dtype=np.float32)
bins = np.arange(2, 36, 4, dtype=np.float32)
...
bkg_eff_bdt_1p = binned_efficiency(nvtx_bdt_1p[~bdt_1p_is_sig], bkg_bdt_pass_1p, bins=bins)
bkg_rej_bdt_1p = 1.0 / bkg_eff_bdt_1p.mean
d_bkg_rej_bdt_1p = bkg_eff_bdt_1p.std / bkg_eff_bdt_1p.mean ** 2

sig_eff_bdt_1p = binned_efficiency(nvtx_bdt_1p[bdt_1p_is_sig], sig_bdt_pass_1p, bins=bins)
...
bkg_eff_bdt_3p = binned_efficiency(nvtx_bdt_3p[~bdt_3p_is_sig], bkg_bdt_pass_3p, bins=bins)
bkg_rej_bdt_3p = 1.0 / bkg_eff_bdt_3p.mean
d_bkg_rej_bdt_3p = bkg_eff_bdt_3p.std / bkg_eff_bdt_3p.mean ** 2

sig_eff_bdt_3p = binned_efficiency(nvtx_bdt_3p[bdt_3p_is_sig], sig_bdt_pass_3p, bins=bins)
...
bkg_eff_1p = binned_efficiency(nvtx_1p[~is_sig_1p], bkg_pass_1p, bins=bins)
bkg_rej_1p = 1.0 / bkg_eff_1p.mean
d_bkg_rej_1p = bkg_eff_1p.std / bkg_eff_1p.mean ** 2

sig_eff_1p = binned_efficiency(nvtx_1p[is_sig_1p], sig_pass_1p, bins=bins)
...
bkg_eff_3p = binned_efficiency(nvtx_3p[~is_sig_3p], bkg_pass_3p,bins=bins)
bkg_rej_3p = 1.0 / bkg_eff_3p.mean
d_bkg_rej_3p = bkg_eff_3p.std / bkg_eff_3p.mean ** 2

sig_eff_3p = binned_efficiency(nvtx_3p[is_sig_3p], sig_pass_3p, bins=bins)
...
ax.set_xlim(0, 34)
ax.set_ylim(50, 300)
ax.set_xlabel("Number of pile-up vertices", ha="right", x=1.0)
ax.set_ylabel("Rejection", ha="right", y=1.0)

ax.errorbar(bin_midpoint, bkg_rej_bdt_1p,
            xerr=bin_half_width, yerr=d_bkg_rej_bdt_1p,
            fmt="o", c="k", label="Optimised BDT", zorder=1)

ax.errorbar(bin_midpoint, bkg_rej_1p,
            xerr=bin_half_width, yerr=d_bkg_rej_1p,
            fmt="o", c="r", label="RNN (Track + Cluster + MLP)", zorder=2)

ax.legend(loc="upper left", fontsize=7)
# ax.text(0.06, 0.94, "60% signal efficiency working point",
#         va="top", fontsize=7, transform=ax.transAxes)

fig.savefig("rnn_tight_bkg_1p.pdf")
...
ax.errorbar(bin_midpoint, sig_eff_bdt_1p.mean,
            xerr=bin_half_width, yerr=sig_eff_bdt_1p.std,
            fmt="o", c="k", label="Optimised BDT", zorder=1)

ax.errorbar(bin_midpoint, sig_eff_1p.mean,
            xerr=bin_half_width, yerr=sig_eff_1p.std,
            fmt="o", c="r", label="RNN (Track + Cluster + MLP)", zorder=2)
...
fig.savefig("rnn_tight_sig_1p.pdf")
...
ax.errorbar(bin_midpoint, bkg_rej_bdt_3p,
            xerr=bin_half_width, yerr=d_bkg_rej_bdt_3p,
            fmt="o", c="k", label="Optimised BDT", zorder=1)

ax.errorbar(bin_midpoint, bkg_rej_3p,
            xerr=bin_half_width, yerr=d_bkg_rej_3p,
            fmt="o", c="r", label="RNN (Track + Cluster + MLP)", zorder=2)

ax.legend(loc="upper left", fontsize=7)
# ax.text(0.06, 0.94, "75% signal efficiency working point",
#         va="top", fontsize=7, transform=ax.transAxes)

fig.savefig("rnn_loose_bkg_3p.pdf")
...
ax.errorbar(bin_midpoint, sig_eff_bdt_3p.mean,
            xerr=bin_half_width, yerr=sig_eff_bdt_3p.std,
            fmt="o", c="k", label="Optimised BDT", zorder=1)

ax.errorbar(bin_midpoint, sig_eff_3p.mean,
            xerr=bin_half_width, yerr=sig_eff_3p.std,
            fmt="o", c="r", label="RNN (Track + Cluster + MLP)", zorder=2)
...
fig.savefig("rnn_loose_sig_3p.pdf")
...
# ax0.legend(loc="center left", fontsize=7)
...
# ax0.set_xlim(20, pt_max)
# ax0.tick_params(labelbottom="off")
# ax0.set_ylabel("Rejection / 1000", ha="right", y=1.0)
# ax0.set_ylim(0, 40)
# #ax0.set_yticks([0, 2000, 4000, 6000, 8000])

# ax0.errorbar(bin_midpoint / 1000.0, bkg_rej_bdt_3p/1000,
#              xerr=bin_half_width / 1000.0, yerr=d_bkg_rej_bdt_3p/1000,
#              fmt="o", c="k", label="Optimised BDT")

# ax0.errorbar(bin_midpoint / 1000.0, bkg_rej_3p/1000,
#              xerr=bin_half_width / 1000.0, yerr=d_bkg_rej_3p/1000,
#              fmt="o", c="r", label="RNN (Track + Cluster + MLP)")
