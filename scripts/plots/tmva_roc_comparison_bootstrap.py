...
def main(args):
    ...
    for i in tqdm(range(n_bootstrap)):
        ...
        eff_ref, rej_ref = roc(y_true[idx], y_ref[idx], sample_weight=w[idx])
        interp = interp1d(eff_ref, rej_ref)

        rej_ref = interp(x)

        for i, y in enumerate(y_add):
            if i not in bootstrap_ratios:
                bootstrap_ratios[i] = []

            eff, rej = roc(y_true[idx], y[idx], sample_weight=w[idx])
            interp = interp1d(eff, rej)
            rej = interp(x)
            ratio = rej / rej_ref

            bootstrap_ratios[i].append(ratio)

    # End bootstrap loop

    means = []
    stds = []
    for i in range(len(bootstrap_ratios)):
        arr = np.array(bootstrap_ratios[i])
    ...
    for i, (roc_interp, score, c) in enumerate(zip(roc_interp_add,
                                                   args.add_score, ["r", "b"])):
        mean = means[i]
        std = stds[i]
        ax1.plot(x, mean, c=c)
        ax1.fill_between(x, mean - std, mean + std, facecolor=c, alpha=0.4)
