...
def partial_dependence_tmva(sig, bkg, model, var, var_idx, sampling, w_sig=None,
                            w_bkg=None):
    lsig = len(sig)
    lbkg = len(bkg)

    X = np.concatenate([sig, bkg])
    if w_sig is not None and w_bkg is not None:
    ...
    for x in sampling:
        ...
        pd.append(np.average(pred, weights=W))
...
def main(args):
    ...
    if not args.mode3p:
        ...
        var_trsf = {
            ...
            "TauJets.ptIntermediateAxis": lambda x: np.minimum(x, 100000.0)
        }
        ...
        var2tex = {
            ...
            "TauJets.ptIntermediateAxis": r"$p_\mathrm{T}^\mathrm{clamp}$ / GeV"
        }
    else:
        variables = ["TMath::Min(TauJets.centFrac, 1.0)",
                     "TMath::Log10(TMath::Max(0.1, TauJets.etOverPtLeadTrk))",
    ...
    W_sig = root2array(args.sigf, treename="CollectionTree", branches="weight")
    X_bkg = root2array(args.bkgf, treename="CollectionTree", branches=variables)
    W_bkg = root2array(args.bkgf, treename="CollectionTree", branches="weight")
    ...
    assert len(X_sig) == len(W_sig)
    np.random.seed(42)
    np.random.shuffle(X_sig)
    np.random.seed(42)
    np.random.shuffle(W_sig)
    X_sig = X_sig[:num, :].copy()
    W_sig = W_sig[:num].copy()
    ...
    np.random.seed(42)
    np.random.shuffle(W_bkg)
    X_bkg = X_bkg[:num, :].copy()
    W_bkg = W_bkg[:num].copy()
    ...
    means = partial_dependence_tmva(X_sig, X_bkg, args.modelxml, variables,
                                    var_idx, sampling_trsf, w_sig=W_sig,
                                    w_bkg=W_bkg)

    xscale = 1.0
    if "mEflowApprox" in args.variable or "massTrkSys" in args.variable or "ptIntermediateAxis" in args.variable:
