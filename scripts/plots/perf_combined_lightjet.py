...
def load_bdt(fsig, fbkg, branchname):
    ...
    bkg_bdt = root2array(fbkg, treename="CollectionTree",
                         branches=[branchname, "TauJets.pt", "TauJets.mu", "TauJets.PartonTruthLabelID", "weight"])
    ...
    pid = np.concatenate([-1111 * np.ones(len(sig_bdt)), bkg_bdt["TauJets.PartonTruthLabelID"]])

    return y, y_true, w, pt, mu, pid
...
def load_rnn(fsig, fbkg, sig_deco, bkg_deco):
    ...
    with h5py.File(fsig, "r", driver="family", memb_size=10*1024**3) as f:
        ...
        pid_sig = -1111 * np.ones(len(pt_sig))
    ...
    with h5py.File(fbkg, "r", driver="family", memb_size=10*1024**3) as f:
        ...
        pid_bkg = f["TauJets/PartonTruthLabelID"][bkg_idx:]
    ...
    pid = np.concatenate([pid_sig, pid_bkg])

    return y, y_true, w, pt, mu, pid


# BDT
y_bdt_1p, y_true_bdt_1p, w_bdt_1p, pt_bdt_1p, mu_bdt_1p, pid_bdt_1p = load_bdt(
    fsig_bdt_1p, fbkg_bdt_1p, branchname_1p)
y_bdt_3p, y_true_bdt_3p, w_bdt_3p, pt_bdt_3p, mu_bdt_3p, pid_bdt_3p = load_bdt(
    fsig_bdt_3p, fbkg_bdt_3p, branchname_3p)

# Trk+Cls+MLP
y_trk_cls_mlp_1p, y_true_1p, w_1p, pt_1p, mu_1p, pid_1p = load_rnn(
    fsig_1p, fbkg_1p, sig_deco_trk_cls_mlp_1p, bkg_deco_trk_cls_mlp_1p)
y_trk_cls_mlp_3p, y_true_3p, w_3p, pt_3p, mu_3p, pid_3p = load_rnn(
    fsig_3p, fbkg_3p, sig_deco_trk_cls_mlp_3p, bkg_deco_trk_cls_mlp_3p)

# Trk+MLP
y_trk_mlp_1p, _, _, _, _, _ = load_rnn(
    fsig_1p, fbkg_1p, sig_deco_trk_mlp_1p, bkg_deco_trk_mlp_1p)
y_trk_mlp_3p, _, _, _, _, _ = load_rnn(
...
mask = ~bdt_1p_is_sig & (0 < pid_bdt_1p & pid_bdt_1p < 5)
bkg_bdt_pass_1p = flat_bdt_1p.passes_thr(pt_bdt_1p[mask], mu_bdt_1p[mask],
                                         y_bdt_1p[mask])

bkg_eff_bdt_1p = binned_efficiency(pt_bdt_1p[mask], bkg_bdt_pass_1p, bins=bins)
...
mask = ~bdt_3p_is_sig & (0 < pid_bdt_3p & pid_bdt_3p < 5)
bkg_bdt_pass_3p = flat_bdt_3p.passes_thr(pt_bdt_3p[mask], mu_bdt_3p[mask],
                                         y_bdt_3p[mask])

bkg_eff_bdt_3p = binned_efficiency(pt_bdt_3p[mask], bkg_bdt_pass_3p, bins=bins)
...
mask = ~is_sig_1p & (0 < pid_1p & pid_1p < 5)
bkg_pass_1p = flat_1p.passes_thr(pt_1p[mask], mu_1p[mask],
                                 y_trk_cls_mlp_1p[mask])

bkg_eff_1p = binned_efficiency(pt_1p[mask], bkg_pass_1p,bins=bins)
...
flat_3p = Flattener(binnings.pt_flat, binnings.mu_flat, 0.45)
...
mask = ~is_sig_3p & (0 < pid_3p & pid_3p < 5)
bkg_pass_3p = flat_3p.passes_thr(pt_3p[mask], mu_3p[mask],
                                 y_trk_cls_mlp_3p[mask])

bkg_eff_3p = binned_efficiency(pt_3p[mask], bkg_pass_3p,bins=bins)
