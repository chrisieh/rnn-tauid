...
def main(args):
    ...
    with h5py.File(args.data, "r", **h5file) as data:
        ...
        hadr_data = Data(x=hadr_data.x[mask], y=hadr_data.y[mask], w=hadr_data.w[mask])

    chrg_train, chrg_test, neut_train, neut_test, hadr_train, hadr_test= \
        train_test_split([chrg_data, neut_data, hadr_data], test_size=args.test_size)

    chrg_preproc = preprocess(chrg_train, chrg_test, chrg_preproc_f)
    neut_preproc = preprocess(neut_train, neut_test, neut_preproc_f)
    hadr_preproc = preprocess(hadr_train, hadr_test, hadr_preproc_f)

    for variables, preprocessing in [(chrg_vars, chrg_preproc),
                                     (neut_vars, neut_preproc),
                                     (hadr_vars, hadr_preproc)]:
    ...
    save_preprocessing(args.preprocessing_hadr, hadr_vars, hadr_preproc)

    # Setup training
    n_classes = chrg_train.y.shape[-1]
    shape_1 = chrg_train.x.shape[1:]
    shape_2 = neut_train.x.shape[1:]
    shape_3 = hadr_train.x.shape[1:]
    ...
    # Start training
    hist = model.fit([chrg_train.x, neut_train.x, hadr_train.x], chrg_train.y,
                     sample_weight=chrg_train.w,
                     validation_data=([chrg_test.x, neut_test.x, hadr_test.x],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--preprocessing-hadr", default="preproc_hadr.h5")
    ...
    parser.add_argument("--hadr-var", default=None)
